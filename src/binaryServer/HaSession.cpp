...
private:
    ...
    // the watchdog polls for the server status
    // Sdk 1.5.2: uaclient/uaclientcpp/uasession.cpp:
    //   values between 1000 ms and 50% SessionTimeout
    static const OpcUa_UInt32 WATCHDOG_INTERVAL = 5000; // in ms
...
HaSession::HaSession(HaSession::Configuration& conf, HaSession::HaSessionCallback& callback)
/* throws MutexException */ {
    ...
    d->serverUrl = UaString("opc.tcp://%1:%2")
            .arg(UaString(d->conf->host.c_str())).arg(d->conf->port);
    UaString nodeName("unknown_host");
    char szHostName[256];
    if (0 == UA_GetHostname(szHostName, 256)) {
        nodeName = szHostName;
    }
    d->applicationUri = UaString("urn:%1:%2:%3").arg(nodeName).arg(companyName).arg(
            productName);
    d->productUri = UaString("urn:%1:%2").arg(companyName).arg(productName);
    d->subscriptionCallback = new HaSessionPrivate::HaSessionSubscriptionCallback(callback);
...
void HaSession::open() /* throws HaSessionException */ {
    ...
    d->subscription = new HaSubscription(*d->session, d->conf->sendReceiveTimeout,
            d->conf->publishingInterval, *d->subscriptionCallback);
    ...
    sessionConnectInfo.nWatchdogTime = d->WATCHDOG_INTERVAL;
    ...
    if (d->conf->username != NULL) {       
        sessionSecurityInfo.setUserPasswordUserIdentity(
            UaString(d->conf->username->c_str()), UaString(d->conf->password->c_str()));
    }
    ...
    try {
        while (!result.isGood()) {
            ...
            result = d->session->connect(d->serverUrl, sessionConnectInfo, sessionSecurityInfo,
                    d /* UaSessionCallback */);
            if (result.isGood()) {
                ...
                if (d->log->isWarnEnabled()) {
                    d->log->warn("Cannot open a connection to %s: %s", d->serverUrl.toUtf8(),
        ...
        d->log->info("Opened connection to %s (sessionId=%s,revisedSessionTimeout=%f,revisedWatchdogInterval=%d)",
                d->serverUrl.toUtf8(), d->session->sessionId().toXmlString().toUtf8(),
                d->session->revisedSessionTimeout(), d->session->watchdogTime());
    } catch (Exception& e) {
        delete d->session;
        d->session = NULL;
        // convert exception to IODataProviderException
        std::ostringstream msg;
        msg << "Cannot open a connection to " << d->serverUrl.toUtf8() << " within "
                << d->conf->connectTimeout << " seconds";
...
void HaSession::close() /* throws HaSessionException, HaSubscriptionException */ {
    ...
    UaStatus result = d->session->disconnect(serviceSettings, OpcUa_True /* deleteSubscriptions*/);
    if (result.isGood()) {
        if (d->log->isInfoEnabled()) {
            d->log->info("Closed connection %s to %s",
                    d->session->sessionId().toXmlString().toUtf8(), d->serverUrl.toUtf8());
        }
        delete d->session;
        d->session = NULL;
    } else {
...
UaStringArray HaSession::getNamespaceTable() const {
    ...
    if (d->session == NULL) {
        throw ExceptionDef(HaSessionException,
                std::string("Cannot get namespace table due to closed session"));
    }
    return d->session->getNamespaceTable();
}

UaStructureDefinition HaSession::getStructureDefinition(const UaNodeId& dataTypeId) {
    MutexLock lock(*d->mutex);
    if (d->session == NULL) {
        throw ExceptionDef(HaSessionException,
                std::string("Cannot get structure definition due to closed session"));
    }
...
/*throws HaSessionException*/ {
    ...
    while (!nodeId.isNull() && nodeId.namespaceIndex() != 0) {
        ...
        UaStatus result = d->session->browse(serviceSettings, nodeId, browseContext,
                continuationPoint, referenceDescriptions);
        if (!result.isGood()) {
            std::ostringstream msg;
            msg << "Cannot get super types for " << typeId.toXmlString().toUtf8()
                    << ": " << result.toString().toUtf8();
            throw ExceptionDef(HaSessionException, msg.str());
        }
...
void HaSession::getMethodArguments(const UaNodeId& methodId, UaArguments& returnInputArguments,
        UaArguments& returnOutputArguments) /* throws HaSessionException */ {
    MutexLock lock(*d->mutex);
    if (d->session == NULL) {
        throw ExceptionDef(HaSessionException,
                std::string("Cannot get method arguments due to closed session"));
    }
    UaClientSdk::ServiceSettings serviceSettings;
    serviceSettings.callTimeout = d->conf->sendReceiveTimeout * 1000;
    if (d->log->isDebugEnabled()) {
        d->log->debug("Getting method arguments for %s", methodId.toXmlString().toUtf8());
    }
    ...
    if (!result.isGood()) {
        std::ostringstream msg;
        msg << "Cannot get method arguments for " << methodId.toXmlString().toUtf8()
                << ": " << result.toString().toUtf8();
        throw ExceptionDef(HaSessionException, msg.str());
    }
}
...
void HaSession::read(const std::vector<const UaNodeId*>& nodeIds,
    ...
    MutexLock lock(*d->mutex);
    if (d->session == NULL) {
        throw ExceptionDef(HaSessionException, std::string("Cannot read due to closed session"));
    }
    // get node classes
    std::vector<OpcUa_UInt32> attributeIds;
    ...
    for (int i = 0; i < returnNodeAttributes.size(); i++) {
        ...
        if (nodeAttr.getException() == NULL) {
            ...
            retNodeAttrMap[nodeAttr.getNodeId().toXmlString().toUtf8()] = &nodeAttr;
        }
    }
    ...
    try {
        ...
        if (readableNodeIds.size() > 0) {
            ...
            for (int i = 0; i < values->size(); i++) {
                ...
                NodeAttributes& retNodeAttr = *retNodeAttrMap[nodeAttr.getNodeId().toXmlString().toUtf8()];
                if (nodeAttr.getValue() != NULL) {
                    retNodeAttr.setValue(new UaVariant(*nodeAttr.getValue()));
                }
                if (nodeAttr.getDataType() != NULL) {
                    retNodeAttr.setDataType(new UaNodeId(*nodeAttr.getDataType()));
                }
                if (nodeAttr.getException() != NULL) {
...
/* throws HaSessionException */ {
    ...
    MutexLock lock(*d->mutex);
    if (d->session == NULL) {
        throw ExceptionDef(HaSessionException, std::string("Cannot write data to closed session"));
    }
    ...
    d->log->info("Writing %d node attributes", attrToWrite.length());
    UaStatus result = d->session->write(serviceSettings, attrToWrite, results, diagnosticInfos);
    if (!result.isGood()) {
        std::ostringstream msg;
        msg << "Cannot read values: " << result.toString().toUtf8();
        throw ExceptionDef(HaSessionException, msg.str());
    }
    d->log->info("Wrote %d node attributes\n", results.length());
    // for each result
...
std::vector<UaVariant*>* HaSession::call(const UaNodeId& methodId, const UaNodeId& objectId,
    ...
    serviceSettings.callTimeout = d->conf->sendReceiveTimeout * 1000;
    if (d->log->isInfoEnabled()) {
        d->log->info("Calling method %s on object %s with %d input arguments...",
                methodId.toXmlString().toUtf8(), objectId.toXmlString().toUtf8(),
                inputArguments.size());
    }
    ...
    UaStatus result = d->session->call(serviceSettings, callRequest, callResult);
    // if calling failed
    if (!result.isGood()) {
        std::ostringstream msg;
        msg << "Cannot call method " << methodId.toXmlString().toUtf8() << " on object "
                << objectId.toXmlString().toUtf8() << ": " << result.toString().toUtf8();
        HaSessionException ex = ExceptionDef(HaSessionException, msg.str());
    ...
    if (d->log->isInfoEnabled()) {
        d->log->info("Called method returned with %d output arguments",
...
/* throws HaSubscriptionException */ {
    ...
    for (int i = 0; i < returnNodeAttributes.size(); i++) {
        ...
        try {
            ...
            if (*attr->getNodeClass() == OpcUa_NodeClass_ObjectType) {
                ...
                VectorScopeGuard<BinaryServerNamespace::EventField> evFieldsSG(evFields);
                if (d->log->isDebugEnabled()) {
                ...
                if (d->log->isDebugEnabled()) {
                    for (int i = 0; i < evFields->size(); i++) {
                        ...
                        d->log->debug("Get event field %-20s nodeId=%s,dataType=%s",
            ...
            if (exception == NULL) {
                std::ostringstream msg;
                msg << "Cannot add node " << attr->getNodeId().toXmlString().toUtf8()
                        << " to subscription";
    ...
    if (exception != NULL) {
        ...
        for (std::map<UaNodeId, std::vector<BinaryServerNamespace::EventField*>*>::const_iterator it
                = returnEventFields.begin(); it != returnEventFields.end(); it++) {
            VectorScopeGuard<BinaryServerNamespace::EventField> eventFieldsSG(it->second);
        }
...
void HaSession::unsubscribe(const std::vector<const UaNodeId*>& nodeIds) /* throws HaSubscriptionException */ {
    MutexLock lock(*d->mutex);
...
void HaSessionPrivate::read(const std::vector<const UaNodeId*>& nodeIds,
        std::vector<OpcUa_UInt32>& attributeIds, std::vector<NodeAttributes*>& returnNodeAttributes)
/* throws HaSessionException */ {
    if (nodeIds.size() == 0) {
        return;
    }
    ...
    for (OpcUa_UInt32 retIndex = 0; retIndex < nodeIds.size(); retIndex++) {
        ...
        returnNodeAttributes.push_back(
                new NodeAttributes(*new UaNodeId(nodeId), true /*attachValues*/));
    }
    ...
    UaDataValues values;
    UaDiagnosticInfos diagnosticInfos;
    log->info("Reading %d node attributes...", attrToReadIndex);
    ...
    log->info("Read %d node attributes", values.length());
    // for each result
    for (OpcUa_UInt32 nodesToReadIndex = 0; nodesToReadIndex < attrToRead.length();
            nodesToReadIndex++) {
        ...
        if (status.isGood()) {
            if (log->isDebugEnabled()) {
                log->debug("Read %s attributeId=%d,value=%s", nodeId.toXmlString().toUtf8(),
            ...
            switch (attrToRead[nodesToReadIndex].AttributeId) {
                ...
                case OpcUa_Attributes_Value:
                    returnNodeAttributes[retIndex]->setValue(new UaVariant(values[nodesToReadIndex].Value));
                    break;
            ...
            if (log->isDebugEnabled()) {
                log->debug("Read %s attributeId=%d,status=%s", nodeId.toXmlString().toUtf8(),
                        attrToRead[nodesToReadIndex].AttributeId,
                        status.toString().toUtf8());
            }
            std::ostringstream msg;
            msg << "Cannot read value for " << nodeId.toXmlString().toUtf8()
                    << " and attributeId " << attrToRead[nodesToReadIndex].AttributeId
                    << ": " << status.toString().toUtf8();
            returnNodeAttributes[retIndex]->setException(new ExceptionDef(HaSessionException, msg.str()));
        }
    }
}
...
/*throws HaSessionException*/ {
    ...
    browseContext.includeSubtype = OpcUa_True;
    browseContext.maxReferencesToReturn = 0;
    browseContext.nodeClassMask = OpcUa_NodeClass_Variable | OpcUa_NodeClass_ObjectType;

    UaByteString continuationPoint;
    UaReferenceDescriptions referenceDescriptions;
    UaStatus result = session->browse(serviceSettings, eventTypeId, browseContext,
            continuationPoint, referenceDescriptions);
    if (!result.isGood()) {
        std::ostringstream msg;
        msg << "Cannot get event fields for " << eventTypeId.toXmlString().toUtf8()
                << ": " << result.toString().toUtf8();
        throw ExceptionDef(HaSessionException, msg.str());
    }
    // add variables
    for (OpcUa_UInt32 i = 0; i < referenceDescriptions.length(); i++) {
        ...
        if (OpcUaId_HasSubtype
            ...
            BinaryServerNamespace::EventField* field = new BinaryServerNamespace::EventField(
                    *new UaNodeId(nodeId), true /*attachValues*/);
            try {
                ...
                if (nodeAttributes != NULL && nodeAttributes->size() > 0) {
                    ...
                    if ((*nodeAttributes)[0]->getException() != NULL) {
                        ...
                        HaSessionException* exception = new ExceptionDef(HaSessionException, msg.str());
                        exception->setCause((*nodeAttributes)[0]->getException());
                        field->setException(exception);
                    } else {
                        field->setQualifiedName(new UaQualifiedName(referenceDescriptions[i].BrowseName));
                        field->setDataTypeId(new UaNodeId(*(*nodeAttributes)[0]->getDataType()));
                    }
                } else {
                    std::ostringstream msg;
                    msg << "Cannot read node attributes of event field "
                            << nodeId.toXmlString().toUtf8() << " of event type "
                            << eventTypeId.toXmlString().toUtf8();
                    field->setException(new ExceptionDef(HaSessionException, msg.str()));
                }
            } catch (Exception& e) {
                std::ostringstream msg;
                msg << "Cannot get data type of event field " << nodeId.toXmlString().toUtf8()
                        << " of event type " << eventTypeId.toXmlString().toUtf8();
                HaSessionException* exception = new ExceptionDef(HaSessionException, msg.str());
                exception->setCause(&e);
                field->setException(exception);
            }
...
void HaSessionPrivate::connectionStatusChanged(OpcUa_UInt32 clientConnectionId,
    ...
    switch (serverStatus) {
        ...
        case UaClientSdk::UaClient::ConnectionWarningWatchdogTimeout:
            log->info("Connection status changed to ConnectionWarningWatchdogTimeout");
            break;
        case UaClientSdk::UaClient::ConnectionErrorApiReconnect:
            log->info("Connection status changed to ConnectionErrorApiReconnect");
            break;
        case UaClientSdk::UaClient::ServerShutdown:
            log->info("Connection status changed to ServerShutdown");
            break;
        case UaClientSdk::UaClient::NewSessionCreated:
            log->info("Connection status changed to NewSessionCreated");
            break;
    }
}
